...
plan: 03
subsystem: testing
tags: [yaml-validation, temperature-capture, 7-segment, rs485, pytest]

# Dependency graph
requires:
  - phase: 01-01
    provides: "Python decode library with SEVEN_SEG_TABLE"
  - phase: 01-02
    provides: "ESPHome YAML config and C++ component"
provides:
...
affects: [02-button-injection, hardware-verification]

# Tech tracking
tech-stack:
  added: []
  patterns:
    - "Structural YAML validation: parsed-dict nesting check + raw-text column-0 check"
    - "Pure-function CLI pattern: testable logic in importable functions, serial I/O in main()"
...
patterns-established:
  - "Structural YAML test: recursive dict check for accidentally nested top-level keys"
  - "Raw YAML text test: column-0 enforcement for known top-level keys"
  - "Pure-function capture script: serial I/O isolated in main(), all logic testable without hardware"
