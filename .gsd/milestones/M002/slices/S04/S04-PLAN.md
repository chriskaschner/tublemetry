# S04: Auto-Refresh Setpoint Keepalive

**Goal:** Add a periodic auto-refresh that keeps the detected_setpoint HA sensor current even when nobody touches the panel. Every 5 minutes (or on first boot when detected_setpoint is NaN), the firmware fires a COOL+WARM two-press sequence to force the display to flash the current setpoint. The set-mode state machine in classify_display_state_() captures the value as normal.
**Demo:** After this: After this: setpoint sensor stays current — firmware presses COOL every 5 minutes when no setpoint has been captured, keeping HA in sync with the controller even when nobody touches the panel.

## Tasks
- [x] **T01: Added REFRESHING phase and auto-refresh keepalive: ButtonInjector fires a net-zero down+up sequence every 5 minutes to force the display to flash the setpoint; 352 tests pass and ESPHome compiles clean.** — Implement the auto-refresh keepalive. Four files change in concert; tests follow the existing Python mirror pattern from test_setpoint_detection.py and test_button_injection.py.
...
```
  - Estimate: 90m
  - Files: esphome/components/tublemetry_display/button_injector.h, esphome/components/tublemetry_display/button_injector.cpp, esphome/components/tublemetry_display/tublemetry_display.h, esphome/components/tublemetry_display/tublemetry_display.cpp, tests/test_auto_refresh.py
  - Verify: uv run pytest tests/ --ignore=tests/test_ladder_cache.py -q 2>&1 | tail -5 && uv run esphome compile esphome/tublemetry.yaml 2>&1 | grep -E '(SUCCESS|ERROR|error|warning)' | tail -10
